			...
			public void actionPerformed(ActionEvent arg0){
				...
				try{
					...
					t1 = new Thread(new Runnable(){
						...
						//使用动态数组存储从数据库调用的元素，以实现动态增长，本待使用集合类解决，但Vector在多线程中运行速度更快些。
			...
			public void actionPerformed(ActionEvent arg0){
				try{
					t2 = new Thread(new Runnable(){
						@SuppressWarnings("deprecation")//下面调用的stop()方法不被编译器赞同，但仍可运行，此功能使用布尔类型进行控制更佳
						public void run(){
							try{
								t1.stop();
								//鄙人此着可谓煞费苦心，JAVA的API中没有从jta中获取指定行文本的方法，但行与行之间必然存在换行符"\n"，
								//故使用split()方法将jta文本一行一行地存入数组，再一行一行地从数组调用即可。
